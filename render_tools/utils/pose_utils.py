...
def query_part_pose_from_joint_qpos(data_path, anno_file, joint_qpos, joints_dict, target_parts, robot: sapien.KinematicArticulation):
    ...
    for link_name, link_dict in target_links.items():
        ...
        assert cur_name == 'root'
        joint_names_to_base = joint_names_to_base[:-1] # remove the last joint to 'root'
...
def compute_rotation_matrix(b1, b2):
    """
    Computes the rotation matrix that rotates box b1 to box b2.
    
    Args:
        b1 (np.array): N x 3 array of points in box b1.
        b2 (np.array): N x 3 array of points in box b2.
    
    Returns:
        R (np.array): 3 x 3 rotation matrix.
    """
    # Compute centroids of boxes
    c1 = np.mean(b1, axis=0)
    c2 = np.mean(b2, axis=0)

    # Compute covariance matrix and do SVD
